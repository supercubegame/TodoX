#!/usr/bin/env node
// 把每条闸门的报告合成一条回写到 GitHub 的评论。
//
// 报告缺失一律算失败：在写出报告之前就崩掉的 job 永远不能看起来像通过 —— 一个
// 会静默失效的监控比没有监控更糟。
//
...
import { renderMarkdown, GATES } from './lib/report.mjs';
...
const header = `${failed ? '❌ **TodoX 验证失败**' : '✅ **TodoX 验证通过**'} — ${passedCount}/${totalCount} 项检查通过 · 提交 \`${sha}\`${runLink}`;
