...
const CHECKS = [
  ...
  //
  // 跨轮实测曾是 x4.84 / x4.88 / x4.95，看着像在往上漂 —— 但每轮只有一个数据点，
  // 而「余量薄」和「其实在抛硬币」从报告上看长得一模一样。所以两个比值各测
  // 两轮、**两轮都断言**（比原来严，不是松），报告里带上两轮的差：那是判断
  // 「在漂」还是「在抖」唯一的依据。实测答案是**在抖**：同一轮里单次新增
  // 测出 x4.85 / x4.86，差 0.01，稳得像石头。跨轮那 0.1 是 runner 之间的差别。
  //
  // 地板这个数**不参与断言**，只印在报告里。所以它错了不会红 —— 而它确实错过
  // 一次：查询那条我照抄了整表排序的 4.3，第一轮实测就测出 x3.79 / x4.04，
  // 低于自己的「地板」。根因是热视图先把 2000 行过滤到 214 行，排序作用在过滤
  // 后的子集上，主导项是那遍 O(n) 过滤而不是 n log n 排序，所以地板是 4.0。
  // 这是「散文里的数字注定漂」的一个实例：没有断言在守的数字，只有等到实测
  // 跟它矛盾才会被发现。
  // ======================================================================
  ['规模比值：数据量 x4，查询与单次写入的耗时都不超过 6 倍（两轮各自断言）', () => {
    ...
    // 两条路径的地板都是 4.0，理由不同：
    //   查询   —— 热视图过滤掉九成，排序只作用在剩下那 214 行上，O(n) 过滤主导。
    //   单次新增 —— 查重扫一遍 + 数组复制一遍，两件都是 O(n)。
    const paths = [
      { label: '查询', floor: 4.0, fn: s => core.selectTodos(s, HOT_VIEW) },
