...
scripts/verify.mjs     快闸门：纯核心 + 静态不变量 + CI 自审（41 条，零依赖）
...
- **fontScale 上下限 80/160** ↔ 滑条的 `min`/`max`/`step` ↔ 闸门里按 6 次
  ArrowRight 从 100 走到 160。改 step 就要改按键次数。
- **`scripts/manifest.json` 的条数** ↔ 三个闸门里 `CHECKS` / `STEPS` 数组长度。
  这是等号断言，不是下限 —— 下限会自己漂，加一条少一条都不会红。
- **`GATES`（scripts/lib/report.mjs）** ↔ workflow 里的 `stdout-<slug>.log`
  与 `report-<slug>` 产物名。快闸门断言这三个集合完全相等。
...
- **macOS 与 Windows 上的真实观感与原生行为**（菜单、Dock、任务栏、高 DPI）。
  CI 只在这两个平台验证「能打出包、产物字节数合理」，行为验收得在真机上做。
- **代码签名、公证、安装包在真机上的安装流程**。需要证书，agent 碰不到。
