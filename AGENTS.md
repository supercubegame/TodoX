...
- **回写用的 bot 邮箱**在 `screenshots.yml` 两处：`git config user.email` 那行，和守卫里
  ...
  循环**，不是红。那个提交信息里的跳过标记已删干净，**别加回来**（见夹具坑最后一条）。
...
- **一个 run 都没有：先看提交信息。** 平台会跳过 commit message 里带跳过标记
  （`skip ci` / `ci skip` / `no ci` 那几个方括号写法）的整条 run。实测踩过，而且就在
  「删掉那个字符串守卫」的 PR 里。症状「零个 run」和「还没开始跑」长得一模一样。
...
- **截图回写的循环守卫从没被真的触发过。** 它现在是身份判断（比字符串硬），但用
  ...
  它防的是「有人换成 PAT」那天。属于「够不着的边界」，只能手动 dispatch 去实测一次。
